...
def get_homepage_posts_for_user(username, limit=20):
    recent_posts_q = """SELECT Posted.*, Account.username
                ...
                ORDER BY Posted.time_created DESC LIMIT %s"""
    ...
    for post in posts:
        post['liked'] = get_likes_count_for_post(post['pid'])

    cursor.close()

    return posts

def get_all_recent_posts(limit=20):
    """
...
def get_recent_posts_from_uid(uid, limit=20):
